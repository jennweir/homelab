...
INVENTORY_TEMPLATE="$SCRIPT_DIR/ansible-playbooks/inventory-coreos-lab.template"
INVENTORY_FILE="$SCRIPT_DIR/ansible-playbooks/inventory"
...
cd "$REPO_ROOT/k8s/installation"

SECRETS_FILE="$TEMP_SECRETS_FILE" ./create-ignition.sh lab

IGNITION_FILE="$SCRIPT_DIR/k8s.ign"
POST_INSTALL_SCRIPT="$SCRIPT_DIR/post-install-fcos-k8s.sh"
